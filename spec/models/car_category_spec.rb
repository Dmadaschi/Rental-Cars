...
RSpec.describe CarCategory, type: :model do
  ...
  context 'validate messages of' do
    it 'presence' do
      ...
      expect(car_category.errors[:daily_rate]).to include(
        'não pode ficar em branco'
      )
      expect(car_category.errors[:car_insurance]).to include(
        'não pode ficar em branco'
      )
      expect(car_category.errors[:third_part_insurance]).to include(
        'não pode ficar em branco'
      )
    end
    ...
    it 'greater_than' do
      car_category = build(:car_category,
                           daily_rate: '0',
                           car_insurance: '0',
